...
export default function Page() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const stored = localStorage.getItem(`addressOwnerEdit-${id}`);
  ...
  const handleSave = () => {
    ...
    if (allFilled) {
      localStorage.setItem(`addressOwnerEdit-${id}`, JSON.stringify(values));
      setSuccessMessage({
        visible: true,
        message: 'Endereço do proprietário salvo com sucesso!'
      });
      router.push(`/dashboard/proprietarios/editar/${id}/contato`);
    }
  };
  ...
  useEffect(() => {
    ...
    if (isComplete) {
      localStorage.setItem(`addressOwnerEdit-${id}`, JSON.stringify(watchedValues));
    } else if (!loadedFromStorage) {
      localStorage.removeItem(`addressOwnerEdit-${id}`);
    }
