...
export default function Page() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const stored = localStorage.getItem(`addressAgencyEdit-${id}`);
      ...
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_URL_API}/agency/${id}`
        );
  ...
  const handleSave = () => {
    ...
    if (allFilled) {
      localStorage.setItem(`addressAgencyEdit-${id}`, JSON.stringify(values));
      setSuccessMessage({
        visible: true,
        message: 'Endereço do imovel salvo com sucesso!'
      });
      router.push(`/dashboard/imobiliarias/editar/${id}/contato`);
    }
  };
  ...
  useEffect(() => {
    ...
    if (isComplete) {
      localStorage.setItem(`addressAgencyEdit-${id}`, JSON.stringify(watchedValues));
    } else if (!loadedFromStorage) {
      localStorage.removeItem(`addressAgencyEdit-${id}`);
    }
