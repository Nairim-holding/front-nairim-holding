...
export default function Page(){
    ...
    useEffect(() => {
        const saved = localStorage.getItem("addressTenant");
        if (saved) {
    ...
    const handleSave = () => {
        if (isFormComplete) {
            localStorage.setItem("addressTenant", JSON.stringify(watchedValues));
            setItem(true);
            setSuccessMessage({
                visible: true,
                message: 'Endereço do inquilino salvo com sucesso!'
            });
            router.push('/dashboard/inquilinos/cadastrar/contato');
        } 
    };
    return (
      <>
